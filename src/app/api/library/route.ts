...
export function serializePlaylist(pl: {
  id: string;
  name: string;
  color: string;
  createdAt: string;
...
export function defaultPlaylists(userId: string) {
  return [
    {
      id: "pw-liked",
      name: "Liked Songs",
      color: "from-purple-600 to-blue-500",
      songIds: db.getLikes(userId).map((l) => l.songId),
      createdAt: db.getLikes(userId)[0]?.likedAt ?? new Date().toISOString(),
    },
    {
      id: "pw-discover",
...
function serializeDownloads(userId: string) {
  return db
    .getDownloads(userId)
    .map((d) => {
    ...
    .filter((s): s is NonNullable<ReturnType<typeof toCard>> & { downloadedAt: string } => s !== null);
}
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const custom = db.getPlaylists(user.id).map((pl) => serializePlaylist(pl));
  const playlists = [
    ...defaultPlaylists(user.id).map((pl) => serializePlaylist(pl, true)),
    ...custom,
  ];
  const downloads = serializeDownloads(user.id);
  const recentSearches = db.getRecentSearches(user.id);
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (body.action === "playlist") {
    ...
    const color =
      body.color ?? PLAYLIST_COLORS[db.getPlaylists(user.id).length % PLAYLIST_COLORS.length];
    const pl = db.createPlaylist(user.id, { name, color });
    if (body.songId) db.addToPlaylist(user.id, pl.id, body.songId);
  ...
  if (body.action === "download") {
    ...
    db.markDownloaded(user.id, song.id);
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {
  ...
  if (songId) db.removeDownload(user.id, songId);
  return NextResponse.json({ ok: true });
}