...
-- ---------------------------------------------------------------------------
-- User-uploaded tracks. The audio binary lives in the "uploads" Storage bucket;
-- preview_url points at its public/signed URL.
-- ---------------------------------------------------------------------------
...
create policy "users manage their own uploaded songs"
  on public.user_songs for all using (auth.uid() = user_id);
